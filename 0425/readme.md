...
`Makefile 구성요소`:
- **목표(Target)**: 빌드할 파일
- **의존성(Dependencies)**: 목표가 생성되기 전에 필요한 파일
- **명령어(Commands)**: 목표를 만들기 위한 명령어
...
| 구분 | 하드 링크 | 심볼릭 링크 |
|------|-----------|-------------|
| 동작 | 파일 이름과 inode를 연결 | 파일 경로 참조 |
| 장점 | 속도 빠름, 원본 삭제 시 링크 유지 | 디렉토리 링크 가능, 외부 파일 참조 가능 |
| 단점 | 다른 파티션에서 팡리 생성 불가, 디렉토리 링크 불가능 | 속도 비교적 느림, 원본 삭제 시 링크 깨짐 |
...
- **PCB에 포함된 주요 정보**: PID, 상태, 레지스터, 메모리 정보, 열린 파일, 스케줄링 정보 등
...
### 풀이
**실제 소유자(Real Owner)**: 파일을 만든 사용자 또는 사용자 계정. </br>
**유효 소유자(Effective Owner)**: 특수 권한을 통해 얻을 수 있는 파일 접근 권한.
- 예를 들어, passwd 명령어는 root가 실제 소유자이지만, 일반 사용자가 사용할 때는 setuid 비트가 설정되어 있어 root 권한을 임시로 받게 됨. 즉, 유효 소유자는 root
