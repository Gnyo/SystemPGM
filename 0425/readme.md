...
### 풀이
**실제 소유자(Real Owner)**: 파일을 만든 사용자 또는 사용자 계정. </bn>
**유효 소유자(Effective Owner)**: 특수 권한을 통해 파일에 접근할 때 사용되는 권한을 가진 사용자.
- passwd 명령어는 root 사용자가 실제 소유자이지만
- 일반 사용자가 실행할 때는 root 권한을 임시로 부여받게 됨, 이때 **유효 소유자는 root**
